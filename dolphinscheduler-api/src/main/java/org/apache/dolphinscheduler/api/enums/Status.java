...
public enum Status {
    ...
    DATASOURCE_CONNECT_REJECT_KEYWORD(10184, "data source connection does not allow the [{0}] keyword",
            "数据源连接参数不允许使用[{0}]关键字"),
    ...
    DELETE_PROCESS_DEFINITION_USE_BY_OTHER_FAIL(10193,
            "delete process definition [{0}] fail, cause used by other tasks: {1}", "删除工作流[{0}]失败，被其他任务引用：{1}"),
    ...
    DELETE_PROCESS_DEFINITION_SCHEDULE_CRON_ERROR(10196, "delete workflow [{0}] schedule error", "删除工作流 [{0}] 调度配置错误"),
    ...
    LISTING_EXECUTING_WORKFLOWS_BY_MASTER_ERROR(10200, "listing executing workflows by master address error",
            "查询Master中运行的工作流实例信息错误"),
    LISTING_DISPATCHING_TASK_INSTANCES_BY_MASTER_ERROR(10201,
            "listing dispatching TaskInstance by master address error", "查询Master中正在分发的任务实例信息错误"),
    LISTING_EXECUTING_TASK_EXECUTION_CONTEXT_BY_WORKER_ERROR(10202,
    ...
    RESOURCE_OWNER_OR_TENANT_CHANGE_ERROR(20021,
            "resource[{0}] owner is deleted or the tenant is unbound, the operation is not allowed",
            "资源[{0}]所属用户被删除或者其租户被解除绑定, 无法进行该操作"),

    RESOURCE_CREATE_ERROR(20022, "resource[{0}] create error",
            "资源[{0}]创建失败"),

    UPLOAD_FOLDER_WARNING(20023,
    ...
    PROCESS_DEFINE_NOT_RELEASE(50004, "process definition {0} process version {1} not on line",
    ...
    TASK_GROUP_USED_SIZE_NOT_EMPTY(130032, "the used size number of task group is not empty", "任务组已使用容量不为空"),
