...
public class ResourcesServiceImpl extends BaseServiceImpl implements ResourcesService {
    ...
    public Result<Object> updateResource(User loginUser,
        ...
        AuthorizationType authorizationType =
                type.equals(ResourceType.FILE) ? AuthorizationType.RESOURCE_FILE_ID : AuthorizationType.UDF_FILE;
        boolean canOperatorPermissions =
                canOperatorPermissions(loginUser, new Object[]{resourceId}, authorizationType, funcPermissionKey);
        if (!canOperatorPermissions) {
            putMsg(result, Status.NO_CURRENT_OPERATING_PERMISSION);
            return result;
        }
    ...
    private Result<Object> fileNameCheck(Result<Object> result, String name) {
        ...
        if (restrictedContents != null) {
            Set<String> containSet = Arrays.stream(restrictedContents.split(Constants.COMMA))
                    .filter(name.toUpperCase(Locale.ROOT)::contains)
    ...
    private Result<Object> fileSuffixCheck(Result<Object> result, ResourceType type, String fileName,
        ...
        String fileAllowTypes = PropertyUtils.getString(Constants.FILE_TYPE_SUPPORT_LIST, null);
        if (fileAllowTypes != null) {
            Set<String> allowCheckSet = Arrays.stream(fileAllowTypes.split(Constants.COMMA))
                    .filter(allowType -> allowType.equalsIgnoreCase(fileSuffix))
                    .collect(Collectors.toSet());
            if (allowCheckSet.isEmpty()) {
    ...
    public Result queryResourceListPaging(User loginUser, int directoryId, ResourceType type, String searchVal,
        ...
        AuthorizationType authorizationType =
                type.equals(ResourceType.FILE) ? AuthorizationType.RESOURCE_FILE_ID : AuthorizationType.UDF_FILE;
        Set<Integer> resourcesIds = resourcePermissionCheckService.userOwnedResourceIdsAcquisition(authorizationType,
    ...
    public Result<Object> delete(User loginUser, int resourceId) throws IOException {
        ...
        AuthorizationType authorizationType =
                resource.getType().equals(ResourceType.FILE) ? AuthorizationType.RESOURCE_FILE_ID
    ...
    public Result<Object> queryResource(User loginUser, String fullName, Integer id, ResourceType type) {
        ...
        AuthorizationType authorizationType =
                type.equals(ResourceType.FILE) ? AuthorizationType.RESOURCE_FILE_ID : AuthorizationType.UDF_FILE;
        boolean canOperatorPermissions =
                canOperatorPermissions(loginUser, new Object[]{resource.getId()}, authorizationType, funcPermissionKey);
        if (!canOperatorPermissions) {
            putMsg(result, Status.NO_CURRENT_OPERATING_PERMISSION);
            return result;
        }
    ...
    public Result<Object> queryResourceById(User loginUser, Integer id) {
        ...
        AuthorizationType authorizationType =
                resource.getType().equals(ResourceType.FILE) ? AuthorizationType.RESOURCE_FILE_ID
                        : AuthorizationType.UDF_FILE;
        boolean canOperatorPermissions =
                canOperatorPermissions(loginUser, new Object[]{id}, authorizationType, funcPermissionKey);
        if (!canOperatorPermissions) {
            putMsg(result, Status.NO_CURRENT_OPERATING_PERMISSION);
            return result;
        }
