...
public class ProcessDefinitionServiceTest {
    ...
    public void testQueryProcessDefinitionList() {
        ...
        Mockito.doThrow(new ServiceException(Status.PROJECT_NOT_FOUND, projectCode)).when(projectService)
                .checkProjectAndAuth(loginUser, project, projectCode, WORKFLOW_DEFINITION);
        try {
            Map<String, Object> map = processDefinitionService.queryProcessDefinitionList(loginUser, projectCode);
            Assert.fail("Should throw service not found exception");
    ...
    public void testQueryProcessDefinitionListPaging() {
        ...
        Mockito.when(processDefineMapper.queryDefineListPaging(
                Mockito.any(IPage.class), Mockito.eq(""), Mockito.eq(loginUser.getId()), Mockito.eq(project.getCode()),
    ...
    public void testBatchMoveProcessDefinition() {
        ...
        Mockito.doNothing().when(projectService).checkProjectAndAuth(loginUser, project1, projectCode,
                TASK_DEFINITION_MOVE);
        Mockito.doNothing().when(projectService).checkProjectAndAuth(loginUser, project2, projectCode2,
                TASK_DEFINITION_MOVE);
    ...
    public void testVerifyProcessDefinitionName() {
        ...
        try {
            Mockito.doThrow(new ServiceException(Status.PROJECT_NOT_FOUND, projectCode)).when(projectService)
                    .checkProjectAndAuth(loginUser, project, projectCode, WORKFLOW_CREATE);
    ...
    public void testGetTaskNodeListByDefinitionCode() {
        ...
        // project check auth fail
        Mockito.doNothing().when(projectService).checkProjectAndAuth(loginUser, project, projectCode, null);
    ...
    public void testViewTree() {
        ...
        Mockito.when(projectMapper.queryByCode(1)).thenReturn(project1);
        Mockito.doNothing().when(projectService).checkProjectAndAuth(loginUser, project1, 1, WORKFLOW_TREE_VIEW);
        Mockito.when(processDefineMapper.queryByCode(46L)).thenReturn(processDefinition);
    ...
    public void testUpdateProcessDefinition() {
        ...
        Project project = getProject(projectCode);
        Mockito.when(projectMapper.queryByCode(projectCode)).thenReturn(getProject(projectCode));
        Mockito.doNothing().when(projectService).checkProjectAndAuth(loginUser, project, projectCode, WORKFLOW_UPDATE);
    ...
    public void testImportSqlProcessDefinition() throws Exception {
        ...
        Project project1 = getProject(projectCode);
        Mockito.when(projectMapper.queryByCode(projectCode)).thenReturn(getProject(projectCode));
        Mockito.doNothing().when(projectService).checkProjectAndAuth(loginUser, project1, projectCode, WORKFLOW_IMPORT);
